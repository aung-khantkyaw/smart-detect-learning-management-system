...
export default function Chat() {
  ...
  useEffect(() => {
    if (chatRoom) {
      fetchMessages();
      // Fetch members immediately to populate UI even if sockets are unavailable
      fetchMembers(chatRoom.id);
      initSocket(chatRoom);
      // Always poll to keep both sides in sync even if no socket broadcast
  ...
  const fetchChatRoom = async () => {
    try {
      // Use specific route by offering id
  ...
  const initSocket = (room) => {
    try {
      ...
      socket.on("connect", () => {
        ...
        // Align with REST members endpoint
      ...
      // If backend emits presence, refresh members; otherwise REST covers
  ...
  const fetchMessages = async () => {
    ...
    try {
      const data = await api.get(`/chat-rooms/COURSE/${chatRoom.id}/messages`); // unwrapped array
